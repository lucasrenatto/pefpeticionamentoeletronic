...
function anexo(e) {
	...
	$('#file').on('change', function(e){
		$('#anexos').append(e.target.value + '\n');
	});
}

function consultaCep(e) {
	$.get( "https://viacep.com.br/ws/"+e.target.value+"/json/", { name: "John", time: "2pm" } )
	.done(function( data ) {
		console.log(data);
		$('#cityInfo').html(data.logradouro + ' - ' + data.bairro + ' - ' + data.localidade);
	});
}

function enviarPeticao() {
	Swal.fire(
		'Petição enviada com sucesso!',
		'Protocolo nº 202009918762367188.',
		'success'
	)
}
