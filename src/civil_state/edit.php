<?php 
    ...
    if(!empty($_GET['Codigo'])){
        $codigo = $_GET['Codigo'];
        $sqlSelect = "SELECT * FROM ESTADO_CIVIL WHERE Codigo=?";
        ...
        $nome = $_POST['Nome'];
        $codigo = $_POST['Codigo'];
        $sqlUpdate = "UPDATE ESTADO_CIVIL SET Nome = ? WHERE Codigo = ?";
...
<body>
    ...
    <div style="margin-top: 85px;">
        <h4 class="custom-form-title">Editar ESTADO CIVIL</h4>
        <hr>
        <form method='POST' action='<?php echo $_SERVER['PHP_SELF']; ?>'>
            ...
            <div class="form-group label-floating">
                <label  class="control-label" for="Nome">Estado Civil:</label>
