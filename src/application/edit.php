<?php 
    ...
    if(!empty($_GET['Codigo'])){
        $codigo = $_GET['Codigo'];
        $sqlSelect = "SELECT * FROM PETICAO_INICIAL_PRIMEIRO_GRAU WHERE Codigo=?";
    ...
    } else if(!empty($_POST['Valor_Acao'])){
        $valor = $_POST['Valor_Acao'];
        $codigo = $_POST['Codigo'];
        $assunto = $_POST['assunto'];
        $sqlUpdate = "UPDATE PETICAO_INICIAL_PRIMEIRO_GRAU SET Valor_Acao = ?, ASSUNTO_PRINCIPAL_Codigo = ? WHERE Codigo = ?";
        $cmd = mysqli_prepare($connection, $sqlUpdate);
        mysqli_stmt_bind_param($cmd,'sii',$valor, $assunto, $codigo);
        mysqli_stmt_execute($cmd);
...
<body>
    ...
    <div style="margin-top: 85px;">
        <h4 class="custom-form-title">Edição DE petição inicial</h4>
        <hr>
        ...
        <div class="form-row">
                <div class="col">
                    <div class="form-group label-floating">
                        ...
                        <input type="number" name='Codigo' class="form-control" id="Codigo" readonly value='<?php echo $data['Codigo'] ?>'>
                    </div>
                </div>
                <div class="col">
                    <div class="form-group label-floating">
                        <label  class="control-label" for="Valor_Acao">Valor da Ação:</label>
                        <input type="number" name='Valor_Acao' class="form-control" id="Valor_Acao" value='<?php echo $data['Valor_Acao'] ?>'>
                    </div>
                </div>
                <div class="col">
                    <div class="form-group label-floating">
                            ...
                            <?php 
                                ...
                                if($response && $numRows > 0)
                                {
                                    while ($line = mysqli_fetch_array($response))
                                    {
                                        $id = $line['Codigo'];
                                        $value = $line['Nome'];
                                        if($id == $data['ASSUNTO_PRINCPAL_Codigo']){
