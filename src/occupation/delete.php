<?php 
    ...
    if(!empty($_GET['Codigo'])){
        $codigo = $_GET['Codigo'];
        $sqlSelect = "SELECT * FROM PROFISSAO WHERE Codigo=?";
        ...
        $codigo = $_POST['Codigo'];
        $sqlUpdate = "DELETE FROM PROFISSAO WHERE Codigo = ?";
        $cmd = mysqli_prepare($connection, $sqlUpdate);
        mysqli_stmt_bind_param($cmd,'s',$codigo);
        mysqli_stmt_execute($cmd);
...
<body>
    ...
    <div style="margin-top: 85px;">
        <h4 class="custom-form-title">Excluir PROFISsão</h4>
        <hr>
